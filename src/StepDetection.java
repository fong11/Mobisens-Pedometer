...
/*	public static final double GRAVITY = 9.80665;
	public static final double MAGNETIC_FIELD = 60.0;
    private final static String TAG = "StepDetector";
    private double   mLimit = 10;
    ...
    public StepDetection() {
        ...
        mYOffset = h * 0.5f;
        mScale[0] = - (h * 0.5f * (1.0f / (GRAVITY * 2)));
        mScale[1] = - (h * 0.5f * (1.0f / (MAGNETIC_FIELD)));
		mLimit = 10; // because below is removed
    }
    
   /* public void setSensitivity(float sensitivity) {
        //mLimit = sensitivity; // 1.97  2.96  4.44  6.66  10.00  15.00  22.50  33.75  50.62
		mLimit = 10; //eventually we want to change this value. set on medium
    }
        ...
        synchronized (this) {
            if (sensor.getType() == Sensor.TYPE_ORIENTATION) {
                ...
                if (j == 1) {
                    float vSum = 0;
                    for (int i=0 ; i<3 ; i++) {
                        final double v = mYOffset + event.values[i] * mScale[j];
                        vSum += v;
                    }
                    int k = 0;
                    float v = vSum / 3;
